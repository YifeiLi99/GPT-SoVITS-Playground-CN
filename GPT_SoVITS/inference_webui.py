...
def get_tts_wav(
    ...
    # [ADD] 计时开始
    import time, wave, contextlib
    t0_all = time.time()
    out_wav_path = "./outputs/last_webui.wav"  # [ADD] 固定输出路径（相对路径）
    ...
    # [ADD] 在第一次 yield 之后计算时间和音频长度（保持原位置，仅修正 dtype）
    t1_all = time.time()
    infer_s = t1_all - t0_all  # 需要你在函数开头有：_t0_all = time.time()
    # 用与前端一致的 int16 缓冲区，避免 float16 触发 soundfile 报错
    int16_wav = (audio_opt * 32767).astype(np.int16)
    # 直接用缓冲区长度算音频时长，避免再从文件读
    dur = len(int16_wav) / float(opt_sr) if opt_sr > 0 else 0.0
    rtf = infer_s / dur if dur > 0 else float("inf")
    print(f"[RTF] Infer={infer_s:.3f}s | Audio={dur:.3f}s | RTF={rtf:.3f}")
    # 可选：安全落盘为 PCM_16（如果你想保存文件）
    try:
        import soundfile as sf
        os.makedirs("./outputs", exist_ok=True)
        sf.write("./outputs/last_webui.wav", int16_wav, opt_sr, subtype="PCM_16")
    ...
    try:
        ...
        safe_text = safe_text[:40].replace("\n", " ")
        with open("./reports/rtf_log.csv", "a", encoding="utf-8") as f:
            f.write(
                f"{time.strftime('%Y-%m-%d %H:%M:%S')},{infer_s:.3f},{dur:.3f},{rtf:.3f},{safe_text}\n"
            )
    except Exception as e:
        print("[RTF] 日志写入失败:", e)
