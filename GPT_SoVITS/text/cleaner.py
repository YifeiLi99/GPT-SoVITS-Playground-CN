...
def clean_text(text, language, version=None):
    ...
    # [ADD] 插桩：返回前打印输入/输出的摘要
    inspect(
        "frontend.clean_text(前端清洗)",
        inp=text,
        norm_text=norm_text,
        phones=phones,
        word2ph=word2ph,
...
def clean_special(text, language, special_s, target_symbol, version=None):
    ...
    for ph in phones[0]:
        ...
        if ph == ",":
            ...
            new_ph.append(ph)
    inspect(
        "frontend.clean_special(前端清洗-特殊情况)",
        special_s=special_s, target_symbol=target_symbol,
        norm_text=norm_text,
        phones_before=phones[0][:20],  # 只展示前几项避免刷屏
        phones_after=new_ph[:20]
    )  # [ADD]
